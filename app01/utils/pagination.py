...
在HTML页面中
    ...
    <script>
        $("#jumpBtn").click(function () {
                $.ajax({
                    url: "/task/jump/?search={{ search }}",
                    type: "get",
                    data: $("#jumpForm").serialize(),
                    dataType: "JSON",
                    success: function (res) {
                        var url = "/task/list/?search=" + res.search + "&page=" + res.page
...
class Pagination(object):
    ...
    def html(self):
        ...
        search_string = """
            <li>
                <form id="jumpForm" style="float: left;margin-left: -1px" method="get">
                    <input name="page"
                           style="position: relative;float:left;display: inline-block;width: 80px;border-radius: 0;"
                           type="text" class="form-control" placeholder="页码">
                    <button id="jumpBtn" style="border-radius: 0" class="btn btn-default" type="button">跳转</button>
                </form>
            </li>
            """
